    ...
    if ($attachments) :
        ...
        foreach ($attachments as $attachment_id) :
            ...
            if ($thumb_id == $attachment_id)
                continue; /* ignore the large featured image */
...
//    <button class="btn btn-default active" type="button">1</button>
...
//            echo "<li><a href='" . get_pagenum_link($paged - 1) . "'>&lsaquo;</a></li>";
        ...
        for ($i = 1; $i <= $pages; $i++) {
            if (1 != $pages && (!($i >= $paged + $range + 1 || $i <= $paged - $range - 1) || $pages <= $showitems )) {
                echo ($paged == $i) ? "<a class='btn btn-default active' href='javascript:void(0)'>$i</a>" : "<a href='" . get_pagenum_link($i) . "' class='btn btn-default'>$i</a>";
            }
        }
