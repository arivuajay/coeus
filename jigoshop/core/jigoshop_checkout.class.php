...
class jigoshop_checkout extends Jigoshop_Singleton {
	...
	protected function __construct () {
		...
		//addded by prakash
		add_action( 'jigoshop_create_account', array( $this, 'create_account' ));
	}
...
//				'group' => '1'
	...
	public function checkout_form_billing() {
		...
		if ( jigoshop_cart::ship_to_billing_address_only() ) {
			echo '<div class="col-md-6"><h3>'.__('Billing &amp; Shipping', 'jigoshop').'</h3></div>';
		} else {
			echo '<div class="col-md-6"><h3>'.__('Billing Information', 'jigoshop').'</h3></div>';
		}
		...
		if ( ! jigoshop_cart::ship_to_billing_address_only() && self::get_options()->get( 'jigoshop_calc_shipping') == 'yes' ) :
            ...
            <div class="col-md-6">
                ...
                <!--<p class="form-row" id="shiptobilling">-->
                    ...
                    <input class="input-checkbox" type="checkbox" name="shiptobilling" id="shiptobilling-checkbox" <?php if ($shiptobilling) : echo $shiptodisplay; endif; ?> /> <?php _e('Ship to billing address?', 'jigoshop'); ?></label>
                <!--</p>-->
                </div>
            </div>
		<?php endif;?>
...
/*		// Registration Form
		if ( $this->show_signup ) {
			echo '<div class="checkout-signup">';
			...
			echo '<input class="input-checkbox" id="create_account"';

			if ( $this->get_value('create_account')) echo 'checked="checked" ';
			echo 'type="checkbox" name="create_account" /> <label for="create_account" class="checkbox">'.$account_label.'</label>';
...
*/	}
	...
	function checkout_form_shipping() {
		...
		if ( ! jigoshop_cart::ship_to_billing_address_only() && self::get_options()->get( 'jigoshop_calc_shipping') == 'yes' ) :
            ...
            <div class="row" id="shippingTitleRow">
                <div class="col-md-6"><h2><?php _e('Shipping Information', 'jigoshop'); ?></h2></div>
	...
	public function field($args)
	{
		...
		if ($args['required']) {
			...
			$input_required = ' input-required form-control';
		}
		...
		switch ($args['type']) {
			case 'country':
				$field .= '<div class="form-row '.implode(' ', $args['class']).'">
			...
			case 'state':
				...
				if (jigoshop_countries::country_has_states($current_cc)) {
					// Dropdown
					$field .= '<select name="'.esc_attr($args['name']).'" id="'.esc_attr($args['name']).'" class="'.esc_attr($input_required).' form-control"><option value="">'.__('Select a state&hellip;', 'jigoshop').'</option>';
					...
					// Input
					$field .= '<input type="text" class="input-text'.esc_attr($input_required).' form-control" value="'.esc_attr($current_r).'" placeholder="'.__('State/Province', 'jigoshop').'" name="'.esc_attr($args['name']).'" id="'.esc_attr($args['name']).'" />';
				}
			...
			case 'postcode':
				...
				$field .= '<div class="form-row '.implode(' ', $args['class']).'">
					<label for="'.esc_attr($args['name']).'" class="'.implode(' ', $args['label_class']).'">'.$args['label'].$required.'</label>
					<input type="text" class="input-text'.esc_attr($input_required).'" name="'.esc_attr($args['name']).'" id="'.esc_attr($args['name']).' form-control" placeholder="'.$args['placeholder'].'" value="'.esc_attr($current_pc).'" rel="'.esc_attr($args['rel']).'" />
				</div>'.$after;
				break;
			case 'textarea':
				$field = '<div class="form-row '.implode(' ', $args['class']).'">
					<label for="'.esc_attr($args['name']).'" class="'.implode(' ', $args['label_class']).'">'.$args['label'].$required.'</label>
					<textarea name="'.esc_attr($args['name']).'" class="input-text'.esc_attr($input_required).'" id="'.esc_attr($args['name']).' form-control" placeholder="'.$args['placeholder'].'" cols="5" rows="2" rel="'.esc_attr($args['rel']).'">'.esc_textarea($this->get_value($args['name'])).'</textarea>
				</div>'.$after;
				break;
			case 'select':
				$field .= '<div class="form-row '.implode(' ', $args['class']).'">
			...
			case 'password':
				$field .= '<div class="form-row '.implode(' ', $args['class']).'">
					<label for="'.esc_attr($args['name']).'" class="'.implode(' ', $args['label_class']).'">'.$args['label'].$required.'</label>
					<input type="'.$args['type'].'" class="input-text'.esc_attr($input_required).' form-control" name="'.esc_attr($args['name']).'" id="'.esc_attr($args['name']).'" placeholder="'.$args['placeholder'].'" value="'.$this->get_value($args['name']).'" rel="'.esc_attr($args['rel']).'" />
				</div>'.$after;
				break;
			default :
				ob_start();
	...
	public static function render_shipping_dropdown()
	{
		if (jigoshop_cart::needs_shipping()){
			...
			$available_methods = jigoshop_shipping::get_available_shipping_methods();
			include(get_template_directory().'/jigoshop/checkout/shipping_dropdown.php');
//			include(JIGOSHOP_DIR.'/templates/checkout/shipping_dropdown.php');
	...
	public function create_account()
	{
		...
		if ( $this->show_signup ) {
			...
			if ( $this->get_value('create_account')) echo 'checked="checked" ';
			echo 'type="checkbox" name="create_account" /> <label for="create_account" class="checkbox" style="padding-left:0px;">'.$account_label.'</label>';
			...
			echo '<div id="shippingTitleRow" class="row">
					<div class="col-md-6"><h2>Create An Account</h2></div>
					<div class="col-md-6"></div>
				  </div>';
